...
#   imageio.imwrite(path, grid)
    imageio.imwrite(path,img_as_ubyte(grid))
...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            m = len(labels)
            D_real_loss = torch.sum((D(real_images)-1)**2)
            ...
            D_fake_loss = torch.sum(D(fake_images)**2)

            # 5. Compute the total discriminator loss
            D_total_loss = 1/(2*m) * (D_fake_loss+D_real_loss)
